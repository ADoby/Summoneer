...
namespace SimpleLibrary
{
    ...
    public class SimplePool_Editor : Editor
    {
        ...
        private const float TabButtonHeight = 30f;
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (TabState.intValue == 0)
            {
                ...
                var value = ThisPrefab.objectReferenceValue;

                EditorGUILayout.PropertyField(ThisPrefab, new GUIContent("GameObject Prefab:"));

                if (value != ThisPrefab.objectReferenceValue)
                {
                    if (PoolName.stringValue == "NewPool" ||
                        string.IsNullOrEmpty(PoolName.stringValue) ||
                        PoolName.stringValue == value.name)
                    {
                        if (ThisPrefab.objectReferenceValue != null)
                            PoolName.stringValue = ThisPrefab.objectReferenceValue.name;
                        else
                            PoolName.stringValue = "NewPool";
