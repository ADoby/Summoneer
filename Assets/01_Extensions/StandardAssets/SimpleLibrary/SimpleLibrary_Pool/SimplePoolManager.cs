...
namespace SimpleLibrary
{
    public class SimplePoolManager : MonoBehaviour
    {
        ...
        #endregion SpecialSingleton

        private Queue<SimplePool.DestroyInfo> destroyQueue = new Queue<SimplePool.DestroyInfo>();
        private Queue<SimplePool.InstantiateInfo> instantiateQueue = new Queue<SimplePool.InstantiateInfo>();
        ...
        /// <returns></returns>
        public static PooledBehaviour Spawn(GameObject prefab, Vector3 localPosition, Quaternion localRotation, Vector3 localScale, bool autoCreatePool = true)
        {
            ...
            foreach (var pool in Instance.Pools)
            {
                if (pool.Uses(prefab))
                {
                    script = pool.Spawn(localPosition, localRotation, localScale);
                    pool.AfterSpawn(script);
                    return script;
                }
            }
            if (autoCreatePool)
            {
                ...
                script = newPool.Spawn(localPosition, localRotation, localScale);
                newPool.AfterSpawn(script);
            }
            else
            {
                var go = GameObject.Instantiate<GameObject>(prefab);

                script = go.GetComponent<PooledBehaviour>();
                if (script == null)
        ...
        public static PooledBehaviour Spawn(PoolInfo info, Vector3 localPosition, Quaternion localRotation, Vector3 localScale)
        {
            var script = Instance.Pools[info.SelectedPoolIndex].Spawn(localPosition, localRotation, localScale);
            Instance.Pools[info.SelectedPoolIndex].AfterSpawn(script);
            return script;
        }

        public static bool Despawn(PooledBehaviour script, bool allowDestroy = true)
        {
            foreach (var pool in Instance.Pools)
            {
                if (pool == null)
                    continue;
                if (pool.TryDespawnObject(script))
                    return true;
            }
            if (allowDestroy)
            {
