...
public class CasterMinionEditor : Editor
{
	...
	private void OnSceneGUI()
	{
		...
		EditorHelper.FourDirectionRange(minion.Body.position, ref range, ref range, ref range, ref range);
		minion.SightRange = range;

		range = minion.AttackRangeX;
		float rangey = minion.AttackRangeY;
		EditorHelper.FourDirectionRange(minion.Body.position + Vector3.up * minion.BodyCenterYDiff, ref range, ref range, ref rangey, ref rangey);
